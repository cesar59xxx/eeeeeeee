  ...
  try {
    ...
    if (req.body.id) {
      ...
      return res.status(400).json({
        error: true,
        message: "Invalid request: 'id' should not be sent",
        details: "The 'id' field is auto-generated by the database",
      })
    }
    ...
    if (!finalTenantId) {
      ...
      if (defaultTenant) {
        ...
        console.log("[v0] Using existing default tenant:", finalTenantId)
      } else {
        ...
        console.log("[v0] Created new default tenant:", finalTenantId)
      }
    }
...
app.get("/api/whatsapp/sessions/:sessionId/status", async (req, res) => {
  try {
    ...
    console.log("[v0] Status response:", { sessionId, status, phoneNumber })

    res.json(response)
  } catch (error) {
    console.error("[v0] Error getting session status:", error)
    res.status(500).json({
      error: true,
